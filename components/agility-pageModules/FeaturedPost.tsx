...
const FeaturedPost: Module<IFeaturedPostModule> = ({module}) => {
	...
	if (featuredPost) {
		// Convert date to string in way that will work on the server and client w/ the same value
		...
		<div className="relative px-8 mb-8">
			{featuredPost && (
				<div className="flex flex-col sm:flex-row max-w-screen-xl mx-auto pt-8 group">
					...
					<div className="sm:w-1/2 lg:w-1/3 bg-gray-100 p-8 border-2 border-t-0 rounded-b-lg sm:rounded-bl-none
					 sm:rounded-r-lg sm:border-t-2 sm:border-l-0 relative">
						<Link href={`/blog/${featuredPost.fields.slug}`} className="cursor-pointer">
